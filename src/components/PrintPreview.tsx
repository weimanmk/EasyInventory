...
export default function PrintPreview({ detail }: Props) {
  const merchantName = useAppStore((state) => state.merchant.name || '我的商行');
  ...
  return (
    <div className="print-preview">
      <Typography.Title level={3} className="print-preview-title">{merchantName}</Typography.Title>
      <Descriptions bordered size="small" column={3} className="print-preview-meta">
        <Descriptions.Item label={terms.customer}>{detail.order.customerName}</Descriptions.Item>
      ...
      <Table
        ...
        columns={[
          ...
          { title: `${terms.product}名称`, dataIndex: 'productName' },
